...
/**
 * 凝神粉末 - 回复50%压力值，可使用10次
 * 
 * @author AI Developer
 * @version 1.0
 * @since 2024
 */
public class CalmPowderItem extends Item {
    private static final int MAX_USES = 10;
    private static final float STRESS_REDUCTION = 0.50f; // 50%压力值回复

    public CalmPowderItem(Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient() && user instanceof net.minecraft.server.network.ServerPlayerEntity serverPlayer) {
            ...
            // 显示使用效果
            user.sendMessage(net.minecraft.text.Text.literal("§a使用凝神粉末，压力值降低50%"), true);
    ...
    private void applyCalmEffect(net.minecraft.server.network.ServerPlayerEntity player) {
        ...
        // 设置宁神效果
        PlayerStatusManager.applyCalmEffect(player, 9000); // 7.5分钟效果
